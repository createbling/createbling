...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/sys/office/form?id=${area.id}&parent.id=${area.parent.id}">节点<shiro:hasPermission name="sys:area:edit">${not empty area.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:area:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="office" name="parent.id" value="${area.parent.id}" labelName="parent.name" labelValue="${area.parent.name}"
					title="节点" url="/sys/area/treeData" extId="${area.id}" cssClass="" allowClear="${area.currentUser.admin}"/>
			</div>
		</div>
